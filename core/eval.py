...
def eval(cfg, log, db_data_loader, q_data_loader, task):
    ...
    iters = [(i, j) for i in range(len(all_q_embs))
    ...
    with tqdm(total=len(iters)) as pbar:
        for i, j in iters:
            pair_recall, pair_similarity, pair_opr = get_recall(
                i, j, all_db_embs, all_q_embs, q_data_loader.dataset.catalog)
    ...
    t = 'Avg. similarity: {:.4f} Avg. recall @N:\n'+str(avg_recall)
    log.info(t.format(avg_similarity))
